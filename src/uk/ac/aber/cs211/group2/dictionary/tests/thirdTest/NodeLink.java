...
public class NodeLink extends AnchorPane {
    ...
    public void bindEnds(DraggableNode source, DraggableNode target) {
        if (source.is_connected() || target.is_connected()) {

        } else {
            if(source.isIn_left() && target.isIn_left() || !source.isIn_left() && !target.isIn_left()){
                ...
                node_link.startYProperty().bind(
                        Bindings.add(source.layoutYProperty(), (source.getWidth() / 2.0) - 50));

                node_link.endXProperty().bind(
                        Bindings.add(target.layoutXProperty(), (target.getWidth() / 2.0)));

                node_link.endYProperty().bind(
                        Bindings.add(target.layoutYProperty(), (target.getWidth() / 2.0) - 50));
                source.registerLink(getId());
                target.registerLink(getId());
                source.set_connected(true);
                target.set_connected(true);
                ThirdTestController.dn_l.add(source);
                ThirdTestController.dn_r.add(target);
            }
        }
    }
